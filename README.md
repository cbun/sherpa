...
- The engine now supports minimum-support variable-order backoff, richer status/freshness reporting, JSON snapshot export, graph maintenance via `gc`, terminal outcome inference from explicit task completion/failure events, and calibrated next-step ranking that considers branch probability, support, and eventual success/failure rates.
