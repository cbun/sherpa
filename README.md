...
- The CLI now also supports `serve`, exposing a small local JSON HTTP daemon at `/health` and `/rpc`.
...
- Add plugin-managed daemon lifecycle for HTTP mode beyond manual `sherpa serve`
